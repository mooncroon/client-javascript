...
class RPClient {
   ...
   */
    ...
    getDeleteLaunchesRequest(launchIds){
        let request = {entities:{}}
        for (let i = 0; i < launchIds.length; i++ ){
            request.entities[launchIds[i]] = {
                "endTime":  this.helpers.now(),
                "status":"STOPPED"}
        }
        return request;
    ...
    finishLaunchesByIDs(launchIds) {
        const request = this.getDeleteLaunchesRequest(launchIds);
        console.log("RP REQUEST:" + JSON.stringify(request))
        const mergeURL = 'launch/stop';
        return this.restClient.update(mergeURL, request, { headers: this.headers })
        .then((response) => {
            this.logDebug(`Launches ${launchIds} were successfully stopped!Response:
            ` +    console.log("RP RESPONSE:" + JSON.stringify(response)));
        })
        .catch((error) => {
            this.logDebug('ERROR');
            this.logDebug(error);
    ...
    getLaunchesIDsByDescription(description){
        const params = new URLSearchParams({
